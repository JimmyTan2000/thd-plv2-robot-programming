...
class State(Enum):
    ...
    REACHED_FOURTH_WALL = auto()
    REACHED_FIFTH_WALL = auto()
    REACHED_SIXTH_WALL = auto()
    ...
    SEVENTH_ROTATION_STOPPED = auto()
    EIGHTH_ROTATION_STOPPED = auto()
...
class Tb3(Node):
    ...
    def odom_callback(self, msg):
        ...
        if self.state == State.REACHED_FOURTH_WALL:
            self.rotate(-15)
            if round(abs(k)) == 90:
                self.rotate(0)
                self.state = State.SIXTH_ROTATION_STOPPED

        if self.state == State.REACHED_FIFTH_WALL:
            self.rotate(15)
            if round(abs(k)) == 0:
                self.rotate(0)
                self.state = State.SEVENTH_ROTATION_STOPPED

        if self.state == State.REACHED_SIXTH_WALL:
            self.rotate(15)
            if round(abs(k)) == 90:
                print("eight r stopped")
                self.rotate(0)
                self.state = State.EIGHTH_ROTATION_STOPPED
    ...
    def scan_callback(self, msg):
        ...
        if self.state == State.FIFTH_ROTATION_STOPPED:
            self.collision_avoidance_sensor(msg, 0.4, State.REACHED_FOURTH_WALL)
        
        if self.state == State.SIXTH_ROTATION_STOPPED:
            self.collision_avoidance_sensor(msg, 0.4, State.REACHED_FIFTH_WALL)

        if self.state == State.SEVENTH_ROTATION_STOPPED:
            self.collision_avoidance_sensor(msg, 0.4, State.REACHED_SIXTH_WALL)

        if self.state == State.EIGHTH_ROTATION_STOPPED:
            #self.go(100)
            if msg.intensities[0] == 2.0:
                print("Red wall reached")
                #self.stop()
                self.state = State.REACHED_RED_WALL
        
        if self.state == State.REACHED_RED_WALL:
            self.collision_avoidance_sensor(msg, 0.4, State.END)
